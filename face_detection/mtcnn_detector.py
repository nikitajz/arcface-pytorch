...
from .helper import non_maximum_suppression, normalize_and_centering_image, detect_first_stage_warpper, \
    detect_first_stage
...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        if model_folder is None:
            ...
            dir = os.path.split(my_path)[0]
            model_folder = os.path.join(dir, "trained_model")
    ...
    def detect_face(self, img):
        ...
        pick = non_maximum_suppression(total_boxes[:, 0:5], 0.7, 'Union')
        ...
        for i in range(num_box):
            ...
            input_buf[i, :, :, :] = normalize_and_centering_image(cv2.resize(tmp, (24, 24)))
        ...
        # nms
        pick = non_maximum_suppression(total_boxes, 0.7, 'Union')
        total_boxes = total_boxes[pick]
        ...
        for i in range(num_box):
            ...
            tmp[dy[i]:edy[i] + 1, dx[i]:edx[i] + 1, :] = img[y[i]:ey[i] + 1, x[i]:ex[i] + 1, :]
            input_buf[i, :, :, :] = normalize_and_centering_image(cv2.resize(tmp, (48, 48)))
        ...
        pick = non_maximum_suppression(total_boxes, 0.7, 'Min')
        total_boxes = total_boxes[pick]
        points = points[pick]
        ...
        for i in range(5):
            ...
            for j in range(num_box):
                ...
                input_buf[j, i * 3:i * 3 + 3, :, :] = normalize_and_centering_image(cv2.resize(tmpim, (24, 24)))
